...
void helper(vector<int>& nums, vector<vector<int>>& ans, vector<int>&temp, int req, int idx){
        ...
        for(int i = idx; i < nums.size(); i++){
            ...
            // check if we need to pick this element 
            ...
            helper(nums, ans, temp, req - nums[i], i + 1);
            temp.pop_back();
...
vector<vector<int>> combinationSum2(vector<int>& candidates, int target) {
    ...
    helper(candidates, ans, temp, req, 0);
