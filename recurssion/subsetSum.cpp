...
treenode * bst_inserttreenode(treenode * root, int d) {if (root == nullptr) {return root = new treenode(d);} if (root->val > d) {root->left = bst_inserttreenode(root->left, d);} else {root->right = bst_inserttreenode(root->right, d);} return root;}
...
void helper(vector<int> nums, set<vector<int>> &ans, int index, vector<int> temp, int currValue, int target){
...
// we need to take this element
	temp.push_back(nums[index]);
	helper(nums, ans, index + 1, temp, currValue + nums[index], target);

	temp.pop_back();

// skipping this element
	helper(nums, ans, index + 1, temp, currValue, target);
...
auto answer(vector<int> nums){
    ...
    helper(nums, ans, index, temp, currValue, target);
