...
int answer_usingSetAndVector(vector<int> &nums){
	set<int> s{};
	vector<int> ans{};
	int ctr = 0;
	for(int i = 0; i<nums.size();i++){
		if(s.find(nums[i]) == s.end()){
			cout << "element not found in the set : " << nums[i] << " \n";
			dbg(s);
			dbg(ans);
			cout << "------------------------------\n";
			s.insert(nums[i]);
...
int answer_onlySet(vector<int> &nums){
 	int size = nums.size();
       set<int> ans {};
       for(auto it : nums){
           ans.insert(it);
...
void solve() {
    ...
    cin >> nums;
    dbg(nums);
    // int ans = answer_usingSetAndVector(nums);
    // int ans = answer_onlySet(nums);
    int ans = answer(nums);
    cout << "---------------------------------\n";
    dbg(ans);
    dbg(nums);
