...
// ------------------------------------------------------------------ solve
int answer(vector<int> vec, int total_hours) {
	dbg(vec);
	int min = 1;
	...
	while (max - min > 1) {
		...
		for (auto it : vec) {
			dbg(it);
			if (it % mid == 0) {
				cout << "-- if --- current hours => " << curr_hours << "... now : ";
				curr_hours += it / mid;
				cout << curr_hours << "\n";

			}
			if (it % mid != 0) {
				cout << "-- else -- current hours => " << curr_hours << "... now : ";
				curr_hours += it / mid;
				curr_hours++;
				cout << curr_hours << "\n";
			}
		}
		cout << "total_hours taken " << curr_hours << ", -> speed " << mid << "\n";
		// break;
		if (curr_hours > total_hours)
			min = mid;
		else
			max = mid;
	}
...
void solve() {
	...
	// 				   0  1  2  3
	int total_hours = 8;
	int ans = answer(vec, total_hours);
