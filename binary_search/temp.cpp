...
int ans(vector<int> arr, int n, int x, int y) {
	...
	dbg(ctrX);
	dbg(ctrY);
	// cout << "printing ctrX > ctrY : " << (ctrX > ctrY) << "\n";
	if (ctrX == ctrY) {
		cout << " --- " << min(x, y) << "\n";
		return min(x, y);
	}
...
void solve() {
	// vector<int>nums = {38 , 49 , 17 , 50, 49, 31 , 73 , 19 , 66 , 96 , 31};
	// vector<int> nums = {1 , 1 , 2 , 2 , 3 , 3 , 4 , 4 , 4 , 4 , 5};
	vector<int> nums = {98, 99, 39, 12, 0, 36, 15, 47, 79, 62, 64};
	int size = nums.size();
	int answer = ans(nums, size, 64, 5);
	dbg(answer);
}
// ------------------------------------------------------------------ main
