...
// ------------------------------------------------------------------ solve
bool answer(vector<vector<int>> &vec, int target) {
	int total_rows = vec.size();
	int total_cols = vec[0].size();
	int rows = 0;
	int cols = total_cols - 1;

	while (rows < total_rows && cols > - 1) {
		int curr = vec[rows][cols];
		if (curr == target)
			return true;
		if (curr < target)
			rows++;
