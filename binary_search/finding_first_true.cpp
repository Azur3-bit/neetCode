...
// ------------------------------------------------------------------ solve
void finding_first_true(vector<bool> nums) {
	dbg(nums);
	int l = 0;
	int r = nums.size() - 1;
	while (r - l > 1) {
...
void even_bin_search(vector<int>nums) {
	int min = 0;
	int max = nums.size() - 1;
	while (max - min > 1) {
...
void solve() {
	// vector<bool> num = {false, false , false , true, true, true , true};
	vector<int> num = { 1, 3 , 5 , 6, 8, 10 , 12};
	//  				0  1   2   3  4  5   6
	// int value = *min_element(num.begin(), num.end());
	// dbg(value);
	// finding_first_true(num);
	even_bin_search (num);
}
// ------------------------------------------------------------------ main
