...
void solve() {
	...
	//  l - begining
	// r - end
	cout << "last element : " << vec[r] << "\n";
	cout << "starting binary searching \n";
	while (l <= r) {  // changed < to <=
		int mid = l + r / 2;
		cout << "mid : " << mid << " elment :" << vec[mid] << "\n";
		if (mid > 0 && target == vec[mid - 1]) {
			cout << "-----------item found\n";
			cout << "index : " << mid - 1 << endl;
			cout << "target element found : " << vec[mid - 1] << "\n";
			return;
		}
		if (l < 0) {
			cout << "l is less than zero -- ERROR \n";
		}
		if (r > 200) {
			cout << "r is greater than 200 -- ERROR \n";
		}
	...
	cout << "outside the while condition \n";
