// search2dMatrix_highlyoptimized_leetcode
// #include <bits/stdc++.h>
#include <iostream>
#include <vector>
#include <string>
...
// ------------------------------------------------------------------ solve
bool ans(vector<vector<int>> vec, int target) {
	dbg(vec);
	dbg(target);
	int row = vec.size();
	int col = vec[0].size();
	int min = 0;
	int max = (vec.size() * vec[0].size()) - 1;
	// dbg(max);
	while (max - min >= 0) {
		int mid = min + (max - min) / 2;
		int mid_row = mid / col;
		int mid_col = mid % col;
		int curr_ele = vec[mid_row][mid_col];
		if (curr_ele == target)
			return true;
		if (curr_ele < target)
			min = mid + 1;
