...
bool possible(vector<int> nums, int days, int m, int k){
	...
	for(auto ele : nums){
		...
		if(flag == false && ele <= days){
			flag = true;

			// cout << "stated at ele : " << ele << "\n";

			count++;
		}
		else if(flag == true && ele > days){
			flag = false;
			req = req - count / k;
	...
	// dbg(req);
	if(req <= 0){
		cout << "possible at day " << days << "\n";
		return true;
	}

	cout << "NOT possible at day " << days << "\n";
	return false;
...
auto answer(vector<int> nums){
    ...
    // n => continous number of bouquets
    ...
    while(low <= high){
    	...
    	if(possible(nums, mid, m, k) == true){
    		/* have a lower day number */
    		ans = min(mid, ans);
    		high = mid - 1;
    	}

    	else{
    		// have a higher day number 
    		low = mid + 1;
    	}

    }

    // bool ans = possible(nums, 0, m,k);
