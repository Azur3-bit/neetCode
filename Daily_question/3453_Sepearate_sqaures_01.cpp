...
auto answer(vector<int> nums){
    ...
    // vector<vector<int>> nums_square = {{0,0,1},{2,2,1}}; // test case 1 

    vector<vector<int>> nums_square = {{0,0,2},{1,1,1}}; // test case 2 

    int ctr = 1;
    int total_area = 0;

    cout << "printing the square coordincate : \n";
    
    for(auto it : nums_square){

    	int it_x = it[0];
    	int it_y = it[1];
    	int it_side = it[2];

    	total_area += it_side * it_side;

    	cout << "area of sqaure - " << ctr++ << " area : " << it_side * it_side << "\n";
