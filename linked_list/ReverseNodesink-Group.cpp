...
void printVals (vector<ListNode * > nums){
...
ListNode * reving(ListNode *head, int k){
	...
	while(tempEnd != nullptr){
		...
		if(ctr == k){
			...
			heads.push_back(tempStart);
			tails.push_back(tempEnd);
		...
		else if(tempEnd->next == nullptr){
			heads.push_back(tempEnd);
	...
	// now reverse particular sections like 
	// reverse(heads[0], tails[0])
	// reverse(heads[1], tails[1])
	// reverse(heads[2], tails[2])

	vector<ListNode *> newHeads {};
	vector<ListNode *> newTails {};

	for(int i = 0; i < tails.size(); i++){
		ListNode * temp = rev(heads[i], heads[i + 1]);



		newHeads.push_back(temp);
	}


	for(auto it : newHeads){
	...
	ListNode * ans = tails[0];


	for(int i = 1; i < newHeads.size(); i++){
	
		newTails[i - 1]->next = newHeads[i];
	...
	// tail connection as well after everything
	if(heads.size() % 2 != 0)
		newTails[(newTails.size()) - 1]->next = heads[(heads.size()) - 1];
...
auto answer(vector<int> nums){
	...
	ListNode * endNode = head->next->next->next;

	rev(head->next->next->next->next, nullptr);
	_showLinkedList(head);	


	rev(head->next->next, endNode->next);
	_showLinkedList(head);	

	rev(head, head->next->next);
	_showLinkedList(head);
...
// if(heads.size() % 2 != 0){
// 	tails[size - 1]->next = heads[size - 1];
// }

//  ## lastNode connection with nullptr 

// heads[size - 1]->next = nullptr
