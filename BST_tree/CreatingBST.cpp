...
node * BST_addNode(node * &root, int da) {
	...
	if (root == nullptr) {
		...
		root->right = BST_addNode(root->right, da);
	}
	else if (root->data > da)
...
// adding node from vector to the BST
void BST_vector(node* &root, vector<int> &nums) {
	cout << "inside BST_vector\n";

	int temp = 0;
	cin >> temp;
	while (temp != -1) {
		root = BST_addNode(root, temp);
		cin >> temp;
	}
	// for (int it : nums)
	// root = BST_addNode(root, it);
	cout << "all the element of nums have been added to the BST \n";
}
void solve() {
