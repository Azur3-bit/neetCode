...
int answer(treenode * root) {
	...
	int leftSum =  answer(root->left);
	int rightSum =  answer(root->right);
	int currSum = leftSum + rightSum + root->data;
	sumVec.push_back(leftSum);
	sumVec.push_back(rightSum);
	sumVec.push_back(currSum);
	sum += max({leftSum, rightSum, root->data, currSum});
	return sum;
}
void solve() {
	vector<int> nums{};
	...
	int ans = answer(root);
	dbg(ans);
	cout << "vecSum : " << *max_element(sumVec.begin(), sumVec.end());
}
// ------------------------------------------------------------------ main
