...
// ------------------------------------------------------------------ solve
treenode * get_node(treenode * root, int p) {
	if (!root)
		return nullptr;
	...
	if (root->val == p)
		return root;
	treenode * left = get_node(root->left, p);
	treenode * right = get_node(root->right, p);
	if (left) {
		if (left->val == p) {
			return left;
		}
...
void solve() {
	...
	treenode * p = get_node(root, p_int);
	treenode * q = get_node(root, q_int);


	cout << "p->val : " << p->val << "\n";
	cout << "q->val : " << q->val << "\n";

	treenode * ans = answer(root, p, q);
	dbg(ans->val);
