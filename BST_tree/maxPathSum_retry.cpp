...
int helper(treenode * root, int total) {
	if (root == nullptr)
		return 0;
	int leftSum = max(0, helper(root->left, total));
	int rightSum = max(0, helper(root->right, total));
	cout << "node : " << root->val << " \t";
	cout << "previous total : " << total << " \t";
	total = max(total, leftSum + rightSum + root->val); // can be rm
	answer = max(answer, leftSum + rightSum + root->val);
	cout << "now total : " << total << " \n";
	// dbg(total);
