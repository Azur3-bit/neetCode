...
// ------------------------------------------------------------------ solve
void get_leaf(treenode *root, vector<int> &leafs) {
	if (root == nullptr)
		return ;
	...
	get_leaf(root->left, leafs);
	get_leaf(root->right, leafs);
}
...
vector<int> answer(treenode * root) {
	...
	// -----  getting all nodes from left side
	treenode * left_side = root;
	vector<int> left_vec {};
	while (left_side) {
		if (left_side->left) {left_vec.push_back(left_side->val); left_side = left_side->left;}
		else {left_vec.push_back(left_side->val); left_side = left_side->right;}
	}
	dbg(left_vec);


	// -----  getting all nodes from right side
	treenode * right_side = root;
	vector<int> right_vec {};
	while (right_side) {
		if (right_side->right) {
			right_vec.push_back(right_side->val);
	...
	//  -------- getting all the lead nodes
	vector<int> leaf_nodes {};
	treenode * leaf_root = root;
	get_leaf(leaf_root, leaf_nodes);
	dbg(leaf_nodes);
	...
	// left side
	for (int i = 0; i < (int)left_vec.size() - 1; i++) {
		ans.push_back(left_vec[i]);
	}
	// leaf nodes
	for (int i = 0; i < (int)leaf_nodes.size() - 1; i++) {
		ans.push_back(leaf_nodes[i]);
	}
	// right side
	for (int i = 0; i < (int)right_vec.size() - 1; i++) {
		ans.push_back(right_vec[i]);
	}
	return ans;
}
