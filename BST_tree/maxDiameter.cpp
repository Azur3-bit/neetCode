...
int depth_right (treenode * root , bool &flag) {
	// if (root->right == nullptr && root->left == nullptr)
...
int answer(treenode * root) {
	...
	int left_depth = depth(root->left, left_flag);
	cout << "----------------- ZZ ----------------------------\n";
	int right_depth = depth_right(root->right, right_flag);
	cout << "----------------- ZZ ----------------------------\n";
	...
	if (left_flag)
		cout << "left side valid \n";
	else
		cout << "left side invalid \n";

	if (right_flag)
		cout << "right side valid \n";
	else
		cout << "rigth side inValid \n";


	int level = min(left_depth, right_depth);
	dbg(level);

	if (left_flag && right_flag) {
		cout << "left_flag && right_flag\n";
		return pow(2, level);
	}
