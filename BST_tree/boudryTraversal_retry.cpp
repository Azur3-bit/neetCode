...
vector<int> traverseBoundary(treenode *root)
{
	...
	vector<int> leftNodes_vec {};
	leftNodes(root, leftNodes_vec);

	dbg(leftNodes_vec);

	// leaf nodes
	vector<int> leafNodes_vec {};
	leafNodes(root, leafNodes_vec);
	dbg(leafNodes_vec);

	// right Nodes
	vector<int> rightNodes_vec {};
	rightNodes(root->right, rightNodes_vec);
	dbg(rightNodes_vec);

	// combining all the vectors
	for (auto it : leftNodes_vec) {
		ans.push_back(it);
	}
	for (int it : leafNodes_vec) {
		ans.push_back(it);
	}
	for (int it : rightNodes_vec)
		ans.push_back(it);
