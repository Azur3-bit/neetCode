...
int get_height(treenode * root) {
	...
	cout << "node : " << root->val << " ";
	cout << "\theight : " << max(left_height, right_height) + 1 << "\n";
	return max(left_height, right_height) + 1;
}
bool isBalanced(treenode* root) {
	if (root == nullptr) {
		cout << "root : " << root->val << " : true\n";
		return true;
	}
	int l_height = get_height(root->left);
	int r_height = get_height(root->right);
	cout << "principal node : " << root->val << " ";
	dbg(l_height);
	dbg(r_height);
	if (abs(l_height - r_height) > 2) {
		cout << "root : " << root->val << " : false\n";
		cout << "height : " << abs(l_height - r_height) << "\n";
		return false;
	}
	else {
		cout << "root : " << root->val << " : true\n";
