...
treenode* createBinaryTree(const vector<int>& nums) {if (nums.empty()) return nullptr;treenode* root = new treenode(nums[0]);queue<treenode*> q;q.push(root);int i = 1;while (i < nums.size()) {treenode* curr = q.front();q.pop();if (i < nums.size() && nums[i] != -1) {curr->left = new treenode(nums[i]);q.push(curr->left);}i++;if (i < nums.size() && nums[i] != -1) {curr->right = new treenode(nums[i]);q.push(curr->right);}i++;}return root;}
// ------------------------------------------------------------------ solve
// calculation for diameter max(left) + max(right) of the root node 
...
int diameter(treenode * main_root){
	cout << "\nanswer function \n";
	...
	helper(root->left, leftmax);
	helper(root->right, rightmax);
