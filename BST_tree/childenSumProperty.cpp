...
// ------------------------------------------------------------------ solve
int helper_bottom_up(treenode * root, bool &flag) {
	if (root == nullptr)
		return 0;

	int left = helper_bottom_up(root->left, flag);
	int right = helper_bottom_up(root->right, flag);
...
bool answer(treenode * root) {
	...
	bool a = helper(root);
	if (a)
		cout << "property statisfed\n";
	else
		cout << "property NOT statisfed\n";
}
void solve() {
	vector<int> v{};
	cin >> v;
